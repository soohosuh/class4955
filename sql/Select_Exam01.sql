...
from emp
order by 3;--3=>3번째 컬럼 기준으로 오름차순을 만든다는 의미or add_sal; 가능
...
select ename, sal, deptno from emp where sal in(2000, 3000) or deptno in(20, 30) order by ename asc;
...
select ename, job, sal from emp where sal not in(1600, 950, 1300);
select ename, job, sal 
from emp --
--where (jop = 'CLEAK' or job = 'SALESMAN') and sal not in (1600, 950, 1300);
--where (job = 'CLEAK' or job = ;SALESMAN') and sal not in(1600, 950, 1300);
where job in('CLEAK', 'SALESMAN') and sal not in(1600, 950, 1300);
